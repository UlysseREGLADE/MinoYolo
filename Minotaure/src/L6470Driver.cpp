...
namespace miam
{
    ...
    L6470::L6470(std::string const& portName, int const& numberOfDevices, int const& busFrequency):
        portName_(portName),
        numberOfDevices_(fabs(numberOfDevices)),
        frequency_(fabs(busFrequency))
    {
    ...
    void L6470::setSpeed(std::vector<double> const& motorSpeeds)
    {
        ...
        for(uint i = 0; i < numberOfDevices_; i++)
        {
            ...
            // Register value.
            uint32_t registerValue = (fabs(speed) * STEPSEC_TO_VELOCITY_REG) + 0.5;
            // Clamp
    ...
    void L6470::moveNSteps(std::vector<int32_t> nSteps)
    {
        ...
        for(uint i = 0; i < numberOfDevices_; i++)
        {
            ...
            // Register value.
            uint32_t registerValue = fabs(nSteps[i]);
            // Clamp
